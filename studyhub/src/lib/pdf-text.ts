// Reads the text of a PDF in the browser: nothing is ever uploaded, so a large file is never sent anywhere and
// costs nothing. pdfjs is loaded on demand, so it does not weigh down any other page.
...
/** Extracts the text of every page. Reports progress so a long file does not look frozen. */
export async function extractPdfPages(file: File, onProgress?: (done: number, total: number) => void): Promise<string[]> {
  ...
  // A scanned document is only pictures: there is no text to read.
  if (pages.join("").replace(/\s+/g, "").length < 50) {
