      ...
      {courses.length === 0 ? (
        ...
        <TableCard>
          <Table caption="Classes">
            <thead>
              <tr><Th>Order</Th><Th>Class</Th><Th>Subjects</Th><Th>Students</Th><Th>Status</Th><Th><span className="sr-only">Actions</span></Th></tr>
            </thead>
            <tbody>
              {courses.map((c, i) => (
                <Tr key={c.id}>
                  <Td><MoveButtons id={c.id} action={moveCourseAction} returnTo={returnTo} first={i === 0} last={i === courses.length - 1} label={c.name} /></Td>
                  <Td>
                  ...
                  <Td className="whitespace-nowrap"><Link href={`/admin/subjects?course=${c.id}`} className="text-primary hover:underline">{plural(c._count.subjects, "subject")}</Link></Td>
                  <Td className="whitespace-nowrap"><Link href={`/admin/students?course=${c.id}`} className="text-primary hover:underline">{plural(c._count.enrollments, "student")}</Link></Td>
